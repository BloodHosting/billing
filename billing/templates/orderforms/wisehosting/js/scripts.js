            ...
            WHMCS.http.jqClient.post(
                ...
                function(data) {
                    if (data) {
                        ...
                        // Remove display:none style from error container
                        jQuery("#containerProductValidationErrorsList").css('display', '');
    ...
    post.done(
        function(data) {
            if (thisRequestId == window.lastSliderUpdateRequestId) {
                // Update #total with the new total
                jQuery("#rawTotals").html(data);

                // Update the order summary
                $('#totals').html($('#rawTotals').find('.summary-totals').html());

                jQuery('#rawTotals').children('.clearfix').each(function(e) {
                    // Check if item has numbers in it bigger then0.00
                    var item = $(this).find('.pull-right').text();
                    // Remove  » from the left text
                    var leftText = $(this).find('.pull-left').text().replace(/»|&nbsp;/g, '');
                    if (leftText.indexOf(': Yes') !== -1) {
                        leftText = leftText.replace(': Yes', '');
                    }
                    // Remove air
                    leftText = leftText.trim();
                    if (item.indexOf('0.00') === -1) {
                        $('#totals').append('<div class="summary-total text-[#ACA9B4] text-sm"><span class="float-left">' + leftText + '</span><span class="float-right fw-bold">' + item + '</span></div>');
                    } else if (item.indexOf('Setup Fee') !== -1) {
                        // Get the value before Setup Fee from item
                        var setupFee = item.split('Setup Fee');
                        // Remove $0.00 USD +
                        setupFee = setupFee[0].replace(/\$0.00 USD \+ /g, '');
                        // Add one time to item
                        leftText = leftText + ' (One Time)';
                        // Add the Setup Fee to the order summary
                        $('#totals').append('<div class="summary-total text-[#ACA9B4] text-sm"><span class="float-left">' + leftText + '</span><span class="float-right fw-bold">' + setupFee + '</span>');
                    }

                });


                // Make the other clearfix element with class text-white and mvoe to bottom
                $('#totals').find('.clearfix').addClass('text-white').addClass('mt-3')
                // Move the monthly to the bottom (can query on text-white and clear fix)
                $('#totals').find('.clearfix.text-white').last().insertAfter($('#totals').find('.summary-total').last());


                // Get .total-due-today and add it to the order summary remove Total Due Today and add Total as first item
                var totalDueToday = $('#rawTotals').find('.total-due-today').html().replace('Total Due Today', '');
                $('#totals').append('<div class="summary-total text-wblue text-xl"><span class="float-left">Due Today:</span><span class="float-right fw-bold">' + totalDueToday + '</span></div>');

                // Find Setup Fees by text and hide it
                $('#totals').find('span:contains("Setup Fees")').parent().remove();
            }
        }
    );
    post.always(
